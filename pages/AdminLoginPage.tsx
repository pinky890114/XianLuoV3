...
const AdminLoginPage: React.FC = () => {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            localStorage.setItem('nocy_admin_auth', 'true');
            navigate('/admin');
        } catch (err: any) {
        ...
        <div className="min-h-screen flex items-center justify-center bg-siam-cream p-4">
            <div className="bg-white p-8 rounded-xl shadow-xl max-w-md w-full text-center">
                <div className="mb-6">
                    ...
                    <p className="text-siam-brown text-sm mt-2">請使用 Google 帳號登入</p>
                </div>
                ...
                <button
                    ...
                    disabled={isLoading}
                    className="w-full bg-white border border-gray-300 text-gray-700 font-bold py-3 px-4 rounded-lg hover:bg-gray-50 transition-all shadow-sm hover:shadow-md disabled:opacity-50 flex items-center justify-center gap-3"
                >
                    {isLoading ? (
                        <span>登入中...</span>
                    ) : (
